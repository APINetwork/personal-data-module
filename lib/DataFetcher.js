...
DataFetcher.prototype.fetch = function( alternateTokenStore, uri, callback ) {
	if( !callback )
	{
		callback = uri;
		uri = alternateTokenStore;
		alternateTokenStore = null;
	}
	...
	if( schemeMatch != null )
	{
		...
		if( scheme == 'apinetwork' ) {
			...
			if( docProducer )
			{
				...
				try {
          			docProducer.produce( 
          				alternateTokenStore ? alternateTokenStore : self.tokenStore,
          				uri, userId, source, resource,
						function( err, rawDoc ) {
