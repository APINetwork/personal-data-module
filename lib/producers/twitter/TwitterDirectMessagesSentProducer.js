...
function TwitterDirectMessagesSentProducer( fetcher ) {
	this.fetcher = fetcher;
	base.init( this );
}
base.inherit( TwitterDirectMessagesSentProducer );

TwitterDirectMessagesSentProducer.prototype.init = function( done ) {
	done();
}
TwitterDirectMessagesSentProducer.prototype.getMatchPatterns = function() {
	return [ '^acct:twitter:[0-9]+', '/direct/sent' ];
}
TwitterDirectMessagesSentProducer.prototype.attemptRequest = function( tokenStore, uri, owner, source, resource, keys, callback ) {
	var self = this;
...
TwitterDirectMessagesSentProducer.prototype.getSomeTweetsPriorTo = function( maxId, userId, keys, callback ) {
	try {
	    ...
	    twit.get('/direct_messages/sent', options,
	      function ( err, data ) {
	        if (err) {
	          callback( err );
	        } else {
...
module.exports = exports = TwitterDirectMessagesSentProducer;
