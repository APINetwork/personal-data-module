...
TwitterTweetIndexProducer.prototype.attemptRequest = function( tokenStore, uri, owner, source, resource, keys, callback ) {
	var self = this;
...
TwitterTweetIndexProducer.prototype.getSomeTweetsPriorTo = function( maxId, userId, keys, callback ) {
	try {
	    ...
	    twit.get('/statuses/user_timeline', options,
	      function ( err, data ) {
	        if (err) {
	          callback( err );
	        } else {
